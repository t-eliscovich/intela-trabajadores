...
html = r.get_data(as_text=True)
check("la pestaña de vacaciones: un número y una frase, sin «ganados»", r.status_code == 200 and "días de vacaciones disponibles" in html and "Este año te tocan" in html and "anados" not in html)
...
check("el que no cumplió el año lee cuándo lo cumple y que se suman 15 de una", "Cumplís un año en Intela el" in html and (HOY - timedelta(days=100)).replace(year=HOY.year + 1).strftime("%d/%m/%Y") in html and "ese día se suman 15" in html)
r = c.get("/yo/perfil")
