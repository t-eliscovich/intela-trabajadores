...
res = V.resumen(ing, tomados=5, ajustes=0, hoy=date(2027, 4, 1), saldo_inicial=4, fecha_saldo=date(2026, 9, 10), dias_por_anio=20)
check("con número fijo por año manda ése: 4 + 20 − 5 = 19", res["saldo"] == 19 and res["dias_proximo_anio"] == 20 and res["fijo"] == 20)
check("número fijo sin saldo inicial: 6 años × 20", V.dias_ganados(ing, date(2026, 3, 15), 20) == 120)
...
r = c.get("/yo/perfil")
check("la pestaña de perfil abre y dice los días por año", r.status_code == 200 and "Vacaciones por año" in r.get_data(as_text=True) and "por ley" in r.get_data(as_text=True))
